...
namespace Supplier2Presta.Service.PriceItemBuilders
{
    public class HappinessPriceItemBuilder : PriceItemBuilderBase, IPriceItemBuilder
    {
        ...
        /*private PriceItem ParseSizeValue(PriceItem result)
        {
            ...
            {
                var sizeFieldValue = result.Size;
                var values = SizeRegex.Matches(sizeFieldValue);
                if(values.Count > 0)
                {
                    ...
                    foreach (Match item in values)
                    {
                        ...
                        size += item.Value.Trim(new[] { '"', ';', '-' }).FirstLetterToUpper().CapitalizeEnglish();
                    }
                    result.Size = size;
                }
                
                values = ColorRegex.Matches(sizeFieldValue);
                if (values.Count > 0)
                {
                    var color = new List<string>();
                    foreach (Match item in values)
                    {
                        if (!item.Value.Equals("безразмерный", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            color += item.Value.Trim(new[] { '"', ';', '-' }).FirstLetterToUpper();
                        }
                    }
                    result.Color = color;
                }
