...
namespace Supplier2Presta.Service.ShopApiProcessors
{
	public class ProductUpdater
	{
		...
		private void UpdateProductPriceAndActivity(PriceItem item, product product)
		{
			...
			var wholesalePrice = product.wholesale_price;
			...
			if (price != product.price)
				Log.Info("Price changed from {0:#.##} to {1:#.##}. Reference: {2}", price, product.price, item.Reference);
			if (wholesalePrice != product.wholesale_price)
				Log.Info("Wholesale price changed from {0:#.##} to {1:#.##}. Reference: {2}", wholesalePrice, product.wholesale_price, item.Reference);
		}
		...
		private void UpdateMetaInfo(PriceItem item, product product)
		{
			if (!SameMetaInfo(item, product))
			{
				...
				_apiFactory.ProductFactory.Update(product);
				Log.Info("Meta info updated. Reference: {0}", item.Reference);
			}
		}
